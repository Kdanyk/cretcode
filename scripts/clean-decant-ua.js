(() => {
  if (window.scanCounterV24_UA) return;
  window.scanCounterV24_UA = true;
  document.querySelectorAll('[data-reit-counter]').forEach((el) => el.remove());
  
  const saveKey = 'scanCounterCDUA_V24State';
  ...
  panel.innerHTML = `
    <div id="mainView">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px solid rgba(0,0,0,0.06); padding-bottom:10px;">
        <div style="font-size:18px; font-weight:900; color:#0f172a; letter-spacing:0.5px; text-transform:uppercase;">REIT+ UA</div>
    ...
    <div id="settingsView" style="display:none">
      ...
      <div style="background:#ffffff; border:1px solid rgba(0,0,0,0.06); border-radius:10px; padding:12px; margin-bottom:10px;">
      ...
      <div style="background:#ffffff; border:1px solid rgba(0,0,0,0.06); border-radius:10px; padding:12px; margin-bottom:10px;">
        <label style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; font-size:11px; font-weight:900; text-transform:uppercase; color:#1e293b;">Залишилось замість суми <input id="leftMode" type="checkbox" style="width:18px; height:18px; accent-color:#3b82f6; cursor:pointer;"></label>
        <label style="display:flex; justify-content:space-between; align-items:center; font-size:11px; font-weight:900; text-transform:uppercase; color:#1e293b;">Авто-колір темпу <input id="autoColor" type="checkbox" style="width:18px; height:18px; accent-color:#3b82f6; cursor:pointer;"></label>
      </div>
      <div style="background:#ffffff; border:1px solid rgba(0,0,0,0.06); border-radius:10px; padding:12px; margin-bottom:10px;">
        <div style="font-size:10px; font-weight:900; text-transform:uppercase; margin-bottom:8px; color:#64748b;">Попередній перегляд</div>
        <div id="miniPreview" style="font-size:20px; font-weight:900; background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; padding:10px; text-align:center; color:#1e293b;">0 | 0.00/год</div>
      </div>
    </div>
  `;
  ...
  function scan() {
    ...
    if (m > p) {
      ...
      if (skipNextPack) {
        diff--;
        skipNextPack = false;
        lastTrigger = 'ПРОПУЩЕНО ПО NLP ' + timeNow();
      }
