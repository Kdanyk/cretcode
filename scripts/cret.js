(() => {
  if (window.scanCounterV29Merged) return;
  window.scanCounterV29Merged = true;
  ...
  function loadState() {
    try {
      ...
      autoLpnEnabled = s.autoLpnEnabled !== undefined ? !!s.autoLpnEnabled : true; // LPN on by default
  ...
  <div id="settingsView" style="display:none; width:100%; box-sizing:border-box;">
    ...
    <div style="background:#ffffff; border:1px solid rgba(0,0,0,0.06); border-radius:10px; padding:12px; margin-bottom:10px; width:100%; box-sizing:border-box;">
      ...
      <label style="display:flex; justify-content:space-between; align-items:center; font-size:11px; font-weight:900; text-transform:uppercase; color:#1e293b; cursor:pointer;">Auto-Przypisz LPN <input id="autoLpnToggle" type="checkbox" style="width:18px; height:18px; accent-color:#10b981; margin:0; cursor:pointer;" ${autoLpnEnabled ? 'checked' : ''}></label>
    </div>
  ...
  panel.querySelector('#autoLpnToggle').checked = autoLpnEnabled; // Чекбокс LPN
  ...
  panel.querySelector('#autoLpnToggle').onchange = (e) => { autoLpnEnabled = e.target.checked; saveState(true); }; // Оновлення стану LPN
